...
class StackedLSTMWithAttention(nn.Module):
    ...
    def forward(self, x, return_attn=False):
        ...
        attn_scores = self.attn_layer(lstm_out)  # [batch, seq_len, 1]
        attn_weights = torch.softmax(attn_scores, dim=1)  # [batch, seq_len, 1]
        context = torch.sum(attn_weights * lstm_out, dim=1)  # [batch, hidden]
        output = self.fc(context)
        if return_attn:
            return output, attn_weights.squeeze(-1)  # 返回注意力权重: [batch, seq_len]
        else:
            return output
...
model = StackedLSTMWithAttention(input_size=len(input_features)).to(device)
